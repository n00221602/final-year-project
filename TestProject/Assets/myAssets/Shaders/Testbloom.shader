Shader "Hidden/Universal Render Pipeline/Bloom"
{
    HLSLINCLUDE
        ...
        // Static overlay texture - hardcoded
        TEXTURE2D(_StaticOverlayTexture);
        SAMPLER(sampler_StaticOverlayTexture);
        ...
        half4 FragUpsample(Varyings input) : SV_Target
        {
            ...
            // Apply static overlay texture using raw screen coordinates (doesn't move with camera)
            half3 overlayColor = SAMPLE_TEXTURE2D(_StaticOverlayTexture, sampler_StaticOverlayTexture, input.texcoord).rgb;
            color *= overlayColor; // Multiplicative blend
            return EncodeHDR(color);
        }
